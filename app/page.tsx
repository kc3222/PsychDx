          ...
          <div className="diagnose-stack">
            <section className="panel">
              ...
              <div className="ccc-stats">
                <div className="stat-box"><span>Symptoms Entered</span><strong>{symptoms.length}</strong></div>
                <div className="stat-box"><span>Risk Level</span><strong className={`risk-${risk.level}`}>{risk.level}</strong></div>
                <div className="stat-box"><span>Top Match</span><strong>{analyzed ? top?.disease.short : "-"}</strong></div>
              </div>
              ...
              <div className="quick-add">
                {QUICK_ADD.map((item) => (
                  <button key={item} type="button" onClick={() => addSymptom(item)}>{item}</button>
                ))}
              </div>
              <div className="symptom-list">
                {symptoms.map((s, idx) => (
                  <div className="symptom-card" key={`${s.text}-${idx}`}>
                    ...
                    {s.expanded ? <div className="timeline-grid">
                      <select value={s.onset} onChange={(e) => setSymptoms((prev) => prev.map((x, i) => (i === idx ? { ...x, onset: e.target.value } : x)))}>
                        <option value="">Onset</option><option value="1w">1 week</option><option value="2w">2 weeks</option><option value="1m">1 month</option><option value="3m">3 months</option><option value="6m">6 months</option><option value="1y">1 year</option><option value="1y+">&gt;1 year</option>
                      </select>
                      <select value={s.frequency} onChange={(e) => setSymptoms((prev) => prev.map((x, i) => (i === idx ? { ...x, frequency: e.target.value } : x)))}>
                        <option value="">Frequency</option><option value="daily">Daily</option><option value="most_days">Most days</option><option value="weekly">Weekly</option><option value="occasional">Occasional</option>
                      </select>
                      <select value={s.pattern} onChange={(e) => setSymptoms((prev) => prev.map((x, i) => (i === idx ? { ...x, pattern: e.target.value } : x)))}>
                        <option value="">Pattern</option><option value="continuous">Continuous</option><option value="episodic">Episodic</option><option value="worsening">Worsening</option><option value="improving">Improving</option>
                      </select>
                    </div> : null}
                  </div>
                ))}
            ...
            <section className="panel">
              ...
              <div className={`risk-banner risk-${risk.level}`}>
                ...
                <span>{risk.level === "emergency" ? "Immediate psychiatric safety assessment recommended." : risk.level === "high" ? "Prompt specialist evaluation recommended." : risk.level === "moderate" ? "Follow-up within 1-2 weeks recommended." : "No major acute risk indicators detected."}</span>
                {risk.triggers.length ? (
                  <div className="risk-trigger-row">
                    {risk.triggers.map((t) => (
                      <span key={t} className="risk-trigger-chip">{t}</span>
                    ))}
                  </div>
                ) : null}
              </div>
              <div className="results-grid">
                {results.map((r) => (
                  <article key={r.disease.id} className={`result-card ${top?.disease.id === r.disease.id ? "top" : ""}`}>
                    <h4>{r.disease.name} <span>{r.pct}%</span></h4>
                    ...
                    <div className="criteria-group"><strong>Confirmed criteria</strong><ul>{r.matched.map((m) => <li key={m}>{m}</li>)}</ul></div>
                    <div className="criteria-group"><strong>Missing to confirm diagnosis</strong><ul>{r.unmatched.slice(0, 4).map((m) => <li key={m}>{m}</li>)}</ul></div>
                  </article>
                ))}
              </div>
            </section>
