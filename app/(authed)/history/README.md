...
Nothing here queries Supabase, so guests and signed-in users see the same page. When History is
built for real, replace `page.tsx` and give the route its own `history.css`.
